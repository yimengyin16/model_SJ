...
# Import projection scale (scale BB-2D) MP2019
...
# Import projection scale (scale BB-2D) in MP2020

MP2020_male_raw <- 
  read_excel(paste0(dir_dataRaw, "MP2020/scale-mp-2020-rates.xlsx"), sheet = "Male", skip = 1) %>% 
  rename(age = 1) %>% 
  filter(!is.na(age)) %>% 
  mutate(age = 20:120, 
         gender = "male")
names(MP2020_male_raw) <- c("age",1951:2036,"gender")

MP2020_female_raw <- 
  read_excel(paste0(dir_dataRaw, "MP2020/scale-mp-2020-rates.xlsx"), sheet = "Female", skip = 1) %>% 
  rename(age = 1) %>% 
  filter(!is.na(age)) %>% 
  mutate(age = 20:120, 
         gender = "female")
names(MP2020_female_raw) <- c("age",1951:2036,"gender")


MP2020_raw <- list(
  male   = MP2020_male_raw,
  female = MP2020_female_raw
)
...
saveRDS(
  MP2020_raw,
  file = paste0(dir_dataOut, "MP2020_raw.rds")
)
