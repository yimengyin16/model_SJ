...
## Notes on the ERC and EEC caps under MEPERS policy
#'   - ADC shared between ER and EE
#'   - Floor of ERC and and EEC: 0 



## Notes on COLA adjustments under MEPERS policy
  ...
  penSim_results <- foreach(k = -2:nsim, .packages = c("dplyr", "tidyr", "purrr")) %dopar% {
    ...
    penSim$UAAL[j]    <- with(penSim, AL[j] - AA[j])
      ...
      if(EEC_type == "sharedNC"){
        # EEC(j) 
        # TEMP for MEPERS
         penSim$EEC[j] <- with(penSim, NC[j] * EEC_sharedNC)
