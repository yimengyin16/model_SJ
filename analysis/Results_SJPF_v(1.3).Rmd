...
```{r stchContribution, include=FALSE}
...
# PV contributions: 2020-2039 (20 years)
df_PVC_20y_disb <- get_PVC(df_results_disb, span = 19, dr = dr)
...
df_rowNames1_disb <- 
	...
	~sim_name, ~sim_label,
	"pf.agg_baseline",     "Baseline: current policy ($million)",
	"pf.agg_disb",         "Lower disability rates (% change from baseline)",
...
df_rowNames2_disb <- 
	...
	~sim_name, ~sim_label,
	"pf.agg_baseline",     "Baseline: current policy",
	"pf.agg_disb",         "Lower disability rates",
	"pf.agg_baselineUpper","Baseline, disb Mort.",
	"pf.agg_disbUpper",    "Lower disability rates, all disb mort."
...
tbl_lowDisbRate <- 
  ...
  fmt_percent(
	...
	tab_header(
    title = md("Estimated impacts of lower disability rates on contributions "),
...
# 	 ) %>% 
  tab_footnote(
    footnote = "Disability rates shown in the table are weighted averages of the rates for fire fighters and police officers.",
    locations = cells_column_spanners(
     "Assumed probability of disability retirement"
    )
 ) %>% 
  tab_footnote(
    footnote = "Disability rates in this scenario are constructed based on the assumed disability rates used by CalPERS for state peace officers and fire fighters (POFF) and police and fire members at public agencies.",
    locations = cells_body(
      columns = c(1),
      rows    = c(2)
    )
