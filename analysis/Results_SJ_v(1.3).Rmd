...
df_results %<>% 
	filter(!str_detect(sim_name, "disb|Upper|disbMort"))
...
## Policy scenarios {.tabset}

### Federated plan {.unnumbered}
...
### Police and Fire plan {.unnumbered}
...
df_C2 <- 
  df_results %>% 
	...
	filter(sim %in% c(0, -2), year %in% c(2021, 2030), str_detect(sim_name, "sj.agg"), !str_detect(sim_name, "highERC") ) %>% 
...
df_rowNames1 <- 
	...
	"sj.agg_combined",     "The two policies above combined"
	)
...
# 	scale_x_continuous(breaks = c(2021, seq(2000, 2100, 5))) + 
# 	#scale_y_continuous(breaks = c(seq(0, 200, 10))) + 
# 	scale_color_manual(values =  colors_manual) +
# 		scale_shape_manual(values = shapes_manual) + 
# 		theme(legend.position = "bottom")
