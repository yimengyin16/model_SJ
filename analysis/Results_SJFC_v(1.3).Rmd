---
title: "San Jose Federated City Employees Retirement Plan: \npreliminary results for discussion"
output:
  html_notebook:
    toc: yes
    number_sections: yes
...
We have modeled the San Jose Federated City Employees' Retirement System under its current policy and under three types of alternative policies. The table below summarizes the key features of these polices.

+-------------------------------+---------------------------+-----------------------------------------------------+----------------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------+------------------------------------------------------------------------+
|                               | Benefit accrual           | Max benefit                                         | COLA                                                           | Employee contribution                                                                                                             | POB                                                                    |
+===============================+:==========================+=====================================================+================================================================+===================================================================================================================================+:=======================================================================+
| **Baseline (current policy)** | Tier1: 2.5%               | 75%(Tier 1) or 70% (Tier 2) of final average salary | Tier1: 3% fixed                                                | Tier1: 27.2% of normal cost                                                                                                       |                                                                        |
|                               |                           |                                                     |                                                                |                                                                                                                                   |                                                                        |
|                               | Tier2: 2%                 |                                                     | Tier2: CPI capped at 1.25% \~ 2% depending on years of service | Tier2: 50% ADC with limit on annual increase; no less than 50% of normal cost (city contribution no less than 50% of normal cost) |                                                                        |
+-------------------------------+---------------------------+-----------------------------------------------------+----------------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------+------------------------------------------------------------------------+
| **Pre-1999 policy**           | 3-year average for tier 1 |                                                     | Tier1: 2.25% fixed                                             |                                                                                                                                   |                                                                        |
|                               |                           |                                                     |                                                                |                                                                                                                                   |                                                                        |
|                               |                           |                                                     | Tier2: no change                                               |                                                                                                                                   |                                                                        |
+-------------------------------+---------------------------+-----------------------------------------------------+----------------------------------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------+------------------------------------------------------------------------+
...
1.  **Pre-1999 policy:** Benefit provisions for new service are generally consistent with the policies before the major benefit changes around 1999.
    ...
    -   Final average salary for tier 1 is calculated based on 3 years of salary 
...
2.  **Cost-sharing policy:**
    ...
    -   The shared-ADC policy of Tier 2 is applied to Tier 1. Under the shared-ADC policy,
        ...
        -   employer contribution cannot be less than 50% of the normal cost.
...
df_rowNames1 <- 
	tribble(
	~sim_name, ~sim_label,
	"fc.agg_baseline",     "Baseline: current policy ($million)",
	#"fc.agg_POB",          "POB for full UAAL",
	"fc.agg_pre99_lowERC", "Pre-1999 benefit",
	"fc.agg_sharedCost",   "Cost-sharing policy",
	"fc.agg_combined",     "The two policies above combined"
	)

df_rowNames2 <- 
	tribble(
	~sim_name, ~sim_label,
	"fc.agg_baseline",     "Baseline: current policy",
	#"fc.agg_POB",          "POB for full UAAL",
	"fc.agg_pre99_lowERC", "Pre-1999 benefit",
	"fc.agg_sharedCost",   "Cost-sharing policy",
	"fc.agg_combined",     "The two policies above combined"
...
df_tierLab <- 
	tribble(
		~tier, ~labels,
		"fc.agg", "Federated total",
		"fc.t1",  "Federated tier 1",
		"fc.t2",  "Federated tier 2"
	)
...
df_fig <- 
	...
	mutate(AL = AL/1e6,
				 policy = str_remove(sim_name,  "fc.agg_|fc.t1_|fc.t2_"),
         tier   = str_extract(sim_name, "fc.agg|fc.t1|fc.t2"),
				 ) %>% 
...
df_fig_shock %>% 
	...
	geom_point() + 
	coord_cartesian(ylim = c(0, 45)) + 
	scale_x_continuous(breaks = c(2021, seq(2021, 2100, 5))) + 
	scale_y_continuous(breaks = c(seq(0, 200, 5))) + 
	scale_color_manual(values = colors_manual) +
		scale_shape_manual(values = shapes_manual) + 
		theme(legend.position = "bottom") + 
...
df_cohort.labels <- 
	tribble(
	~cohort, ~labels,
	"fc_t1_1991_25_60", "Tier 1 retiree\ncurrent age 55",
	"fc_t1_1996_25_60", "Tier 1 active\ncurrent age 50",
	"fc_t1_2011_25_60", "Tier 1 active\ncurrent age 35",
	"fc_t2_2021_25_60", "Tier 2 active\ncurrent age 25"
	)
...
val_pf_baseline <- readRDS(paste0(dir_val,   "val_fc_baseline.rds"))
val_pf_pre99    <- readRDS(paste0(dir_val,   "val_fc_pre99.rds"))
val_pf_sharedCost <- readRDS(paste0(dir_val, "val_fc_sharedCost.rds"))




get_fc.t1 <- function(df, policyFn, tierFn = "t1", planFn = "fc"){
...
df$indivLiab$fc.t1$active %>% 
	mutate(start_year = year - (age - ea)) %>% 
	filter( (start_year == 1991 & ea == 25 & age == 60)|
				  (start_year == 1996 & ea == 25 & age == 60)|
					(start_year == 2011 & ea == 25 & age == 60)
				 ) %>%
...
get_fc.t2 <- function(df, policyFn, tierFn = "t2", planFn = "fc"){

# df <- val_pf_baseline
# policyFn = "Baseline"
# tierFn   = "tier 1"

df$indivLiab$fc.t2$active %>% 
	mutate(start_year = year - (age - ea)) %>% 
	filter( (start_year == 2021 & ea == 25 & age == 60)
				 ) %>%
...
df_indiv <- 
	bind_rows(
		get_fc.t1(val_pf_baseline,  "baseline"), 
		get_fc.t1(val_pf_pre99,     "pre99"),  
		get_fc.t1(val_pf_sharedCost,  "sharedCost"),
		get_fc.t1(val_pf_pre99,     "combined"),
		
		get_fc.t2(val_pf_baseline,  "baseline"), 
		get_fc.t2(val_pf_pre99,     "pre99"),  
		get_fc.t2(val_pf_sharedCost,  "sharedCost"),
		get_fc.t2(val_pf_pre99,     "combined"),  
	) 
...
idx_cohort_fc.t1 <- c(
    paste(c("fc", "t1", "1991", "25", "60"), collapse = "_"),
    paste(c("fc", "t1", "1996", "25", "60"), collapse = "_"),
    paste(c("fc", "t1", "2011", "25", "60"), collapse = "_")
    )


idx_cohort_fc.t2 <- c(
    paste(c("fc", "t2", "2021", "25", "60"), collapse = "_")
    )
...
idx_full <- 
	expand_grid(
		sim = idx_sim, 
		idx_cohort = c(idx_cohort_fc.t1, idx_cohort_fc.t2),
		idx_policy = idx_policy_fc
	)
...
df_ben_sum <- 
	...
	mutate(B_pv  = B_pv / (1 + infl)^(year_ret -  init_year_indiv),
				 B_a60 = B_a60 / (1 + infl)^(year_ret - init_year_indiv),
...
df_ben_sum %>% 
	filter(sim == 0) %>% 
	select(cohort, policy, sim, B_a60, B_a80) %>% 
...
caption_cohort <- c("Note: all members entered the plan at age 25 and retire at age 60 with 35 years of service")
...
fig_benPV <- 
  ...
  labs(
		   title = "San Jose Federated plan\nPresent value of total benefit over age 60 to 85\nunder alternative policies",
...
df_ben_sum %>% 
	filter(sim == 0) %>% 
	select(cohort, policy, sim, B_a60) %>% 
	ggplot(aes(x = cohort, y = B_a60, fill = policy)) + theme_bw() + 
	geom_bar(position = "dodge", stat = "identity") +
	coord_cartesian(ylim = c(0, 150000)) + 
	scale_fill_manual(values = colors_ERC) + 
	scale_y_continuous(labels = scales::comma ) +
		labs(
		   title = "San Jose Federated plan\nInflation adjusted benefit at age 60 under alternative policies",
...
salary_fc.t1 <- 
  val_pf_baseline$salary_full$fc.t1 %>% 
	mutate(tier = "t1", plan = "fc") %>% 
	rename(sx.t1 = sx)

salary_fc.t2 <- 
  val_pf_baseline$salary_full$fc.t2 %>% 
	mutate(tier = "t2", plan = "fc") %>% 
	rename(sx.t2 = sx)
...
df_EECindiv <- 
	expand_grid(idx_full, age = 20:59) %>% 
	separate(idx_cohort, into = c("plan", "tier", "start_year", "ea","retAge"),convert = TRUE) %>% 
	separate(idx_policy, into = c("policy","ERCType"),convert = TRUE) %>% 
	mutate(year = start_year + (age - ea)) %>% 
	left_join(salary_fc.t1) %>% 
	left_join(salary_fc.t2) %>% 
