...
df$indivLiab$pf.t1$active %>% 
	...
	filter( (start_year == 1985 & ea == 25 & age == 60)|
				  (start_year == 1995 & ea == 25 & age == 60)|
					(start_year == 2010 & ea == 25 & age == 60)
				 ) %>% 
	# filter(age == 60) %>% 
...
df_Bx <- 
	bind_rows(
		...
		get_pf.t1(val_pf_measureB,  "sharedCost")
	)  
...
idx_cohort_pf.t1 <- c(
    paste(c("pf", "t1", "1985", "25", "60"), collapse = "_"),
    paste(c("pf", "t1", "1995", "25", "60"), collapse = "_"),
    paste(c("pf", "t1", "2010", "25", "60"), collapse = "_")
    )
...
df_ben %<>% 
	mutate(cola_cum = lag(cumprod(1+cola_actual), 1, 1),
